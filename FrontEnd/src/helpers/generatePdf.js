...
// to calculate the height of section
...
const drawPage = (pageData, pageHtml) => {
  ...
  pageHtml.insertAdjacentHTML("beforeend", '<div class="page-break"></div>');
};
  ...
  if (sectionChidrenClone?.length) {
    for (let subSec of sectionChidrenClone) {
      ...
      // continued logic
        ...
        if (pageHeight - totalHeight > 100) {
          const innerCard = handleProfileSubSections(subSec, pageData, pageHeight)
...
const handleProfileSubSections = (subSec, pageData, pageHeight) => {
